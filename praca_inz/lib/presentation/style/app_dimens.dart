class AppDimens {
  ///0.0
  static const zero = 0.0;

  ///1.0
  static const one = 1.0;

  ///2.0
  static const xxs = 2.0;

  ///4.0
  static const xs = 4.0;

  ///8.0
  static const s = 8.0;

  ///12.0
  static const sm = 12.0;

  ///16.0
  static const m = 16.0;

  ///20.0
  static const ml = 20.0;

  ///24.0
  static const l = 24.0;

  ///32.0
  static const xl = 32.0;

  ///40.0
  static const xxl = 40.0;

  ///48.0
  static const xxxl = 48.0;

  ///56.0
  static const c = 56.0;

  ///64.0
  static const xc = 64.0;

  ///72.0
  static const xxc = 72.0;

  ///80.0
  static const xxxc = 80.0;

  ///100.00
  static const xxxxc = 100.0;
  //120.0
  static const d = 120.0;
  //120.0
  static const g = 200.0;
  //6.0
  static const six = 6.0;

  ///10.0
  static const ten = 10.0;

  static const logoSize = 155.0;

  static const launchLogoSize = 146.0;

  static const phoneIconSizeWithPadding = 96.0;

  static const duration0 = Duration(seconds: 0);

  /// 150 ms
  static const duration150ms = Duration(milliseconds: 150);

  /// 300 ms
  static const duration200ms = Duration(milliseconds: 200);

  /// 300 ms
  static const duration300ms = Duration(milliseconds: 300);

  /// 500 ms
  static const duration500ms = Duration(milliseconds: 500);

  /// 800 ms
  static const duration800ms = Duration(milliseconds: 800);

  /// 1 s
  static const duration1s = Duration(seconds: 1);

  /// 2 s
  static const duration2s = Duration(seconds: 2);

  /// 2 s
  static const duration5s = Duration(seconds: 5);

  /// 2 s
  static const duration10s = Duration(seconds: 10);

  static const onbordingImageHeight1 = 355.0;

  static const onbordingImageWidth1 = 290.0;

  static const onbordingImageHeight2 = 385.0;

  static const onbordingImageWidth2 = 310.0;

  static const onbordingImageHeight3 = 520.0;

  static const onbordingImageWidth3 = 327.0;

  static const superChanceWidth = 27.0;

  static const navbarHeight = 92.0;

  static const headerHeight = 128.0;
  
  static const systemsHeaderHeight = 136.0;

  static const underlinePadding = -3.0;

  static const systemElementHeight = 112.0;

  static const fixedTextSize = 1.25;

  static const tagWidgetHeight= 1.25;
}
